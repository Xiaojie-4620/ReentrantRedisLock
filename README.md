...
使用同一个 ReentrantRedisLock 实例；

调用者是同一个 asyncio.Task；
...
非持有者 Task 释放或续期、上下文获取失败

LockNotOwnedError

Redis 锁已过期、token 已变化或当前客户端已经失去所有权
...
Redis key 被删除或 token 被替换后的丢锁处理；
